...
2. **Thread that signal into `Checker.Check`.** The `Checker` gains access to the lookup (either a field on `Checker` or by passing `profiles.Profile` metadata that already carries it). `classifyFailure` becomes context-aware: signature `classifyFailure(stderr []byte, hasSessionToken bool)`.
